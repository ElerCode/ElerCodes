...
def traversal(node, typedict, id2hash):  # Recursively traverse all nodes
    if node.children:
...
def one_feature(path):
    ...
    id2hash = {}
    id2number = {}
    HashListArray = get_hash(childrendict, nodelist, typedict, id2hash, id2number)
...
def compare(pair1, pair2):
    datanode = []
    # Nodes that no longer need to be compared
    delnode1 = set()
    delnode2 = set()
    ...
    union = len(id2hash1) + len(id2hash2) - intersection  # Union
    sim = float(intersection) / union if union != 0 else 0  # Similarity
