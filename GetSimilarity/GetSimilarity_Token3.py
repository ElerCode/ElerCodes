...
def get_similarity(block1_path, block2_path):
    ...
    while(line1 < block1_len):
        k1 = block1[line1]     # Originally intended to get its hash value to save memory, but not needed here when calculating similarity between two blocks
...
def getCodeBlock(file_path):
    ...
    with open(file_path, 'r') as temp_file:
        ...
        for line in lines:
            ...
            temp_line = ''   # Concatenate all elements in a line and treat it as a single token
            for token in tokens:
...
def get_filter_similarity(block1_path, block2_path, N):
    '''Only differs from nil in the last line'''
    block1 = getCodeBlock(block1_path)
    block2 = getCodeBlock(block2_path)
    common_ngram = 0
    invertedIndex = {} # Record the block indices containing each n-gram
