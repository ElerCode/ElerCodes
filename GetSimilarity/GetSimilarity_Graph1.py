...
def Levenshtein(p,q):
    ...
    # the edit distance is obtained step by step according to the state transition equation
...
def LCS(s1, s2):
    m = [[0 for i in range(len(s2) + 1)] for j in range(len(s1) + 1)]  # Generate a zero matrix, with one extra column than the string length for easier calculation
    mmax = 0  # Length of the longest match
                     # The last position in s1 corresponding to the longest match
    for i in range(len(s1)):
        for j in range(len(s2)):
            if s1[i] == s2[j]:  # If equal, add to the existing common substring
                m[i + 1][j + 1] = m[i][j] + 1
                if m[i + 1][j + 1] > mmax:
                    mmax = m[i + 1][j + 1]
    return mmax  # Return the longest substring and its length


def delt(simp, simq):  # simp and simq are 2D lists
    sim_total = 0
    len_p = len(simp)  # Cardinality of p
    len_q = len(simq)  # Cardinality of q
    if len_p > len_q:
        for index_q in range(len_q):  # Divide by simQ
            max_sim = 0
            for index_p in range(len_p):
                tmp = LCS(simp[index_p], simq[index_q])  # Find the min delt(p,q), can be replaced with other functions
        ...
        for index_p in range(len_p):  # Divide by simP
            max_sim = 0
            for index_q in range(len_q):
                tmp = LCS(simp[index_p], simq[index_q])  # Find the min delt(p,q)
